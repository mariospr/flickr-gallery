...
if ( defined('CODEMEME_TEST') ) {
	...
	define('CM_FLICKR_GALLERY_VERSION', '1.5.3-dev1');
}
...
class DC_FlickrGallery {
    ...
    function init() {
		...
		if ( !is_admin() ) {
			...
			if ( DC_FlickrGallery::get_major_version() >= 2.8 ) {
				...
				wp_enqueue_style('fg-jquery-ui', DC_FlickrGallery::getURL() . 'tab-theme/jquery-ui-1.5.3-dev1.css', array(), '1.5.2', 'all');
			}
											...
											(function($){
												$(document).ready(function(){
													$("#photostream-next a, #photostream-prev a").click(function(e){
														...
														$.post("<?php echo apply_filters('fg-ajax-endpoint', get_bloginfo('wpurl')) ?>", {
											...
											(function($){
												$(document).ready(function(){
													function flickr_gallery_load_photoset($display, id, page) {
														...
														$display.load("<?php echo apply_filters('fg-ajax-endpoint', trailingslashit(get_bloginfo('wpurl'))) ?>?action=flickr-gallery-photoset&pagination=<?php echo $attr['pagination'] ?>&page=" + page + "&id=" + id, null, function(){
															<?php if ( get_option('fg-flightbox') === false || get_option('fg-flightbox') ) : ?>
	...
	function header() {
		if ( get_option('fg-flightbox') === false || get_option('fg-flightbox') ) {
			?>
				<script type="text/javascript">
					var get_sizes = null;
						...
						(function($){
							get_sizes = function(id){
								...
								$.ajax({
									...
									dataType: "json",
									type: "POST",
									url: "<?php echo apply_filters('fg-ajax-endpoint', trailingslashit(get_bloginfo('wpurl'))) ?>",
									success: function(rsp) {
										sizes = rsp;
									}
								});
